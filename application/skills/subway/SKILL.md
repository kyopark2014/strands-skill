...
## US Subway Information (Beta) / 미국 지하철 정보 (베타)

When the user asks for **US subway/metro** info, use this section.

사용자가 **미국 지하철/메트로** 정보를 요청하면 이 섹션을 사용합니다.

### Scope / 범위

- This skill remains Seoul-first.
- For US requests, provide **city-specific** results using the APIs below.
- If API behavior changes or a city is unsupported, verify current endpoints with Tavily/web search first.

이 스킬은 기본적으로 서울 지하철용입니다.  
미국 요청은 아래 도시별 API로 처리하고, 동작이 불확실하면 Tavily/웹 검색으로 최신 엔드포인트를 먼저 확인합니다.

### Supported US Data Sources / 지원 미국 데이터 소스

#### 1) MBTA (Boston) - No API key required

- Docs: [https://api-v3.mbta.com/docs/swagger/index.html](https://api-v3.mbta.com/docs/swagger/index.html)
- Realtime predictions:
  - `GET https://api-v3.mbta.com/predictions?filter[stop]={stop_id}&include=trip,route`
- Subway routes:
  - `GET https://api-v3.mbta.com/routes?filter[type]=0,1`
- Stops search (name):
  - `GET https://api-v3.mbta.com/stops?filter[name]={name}`

#### 2) BART (San Francisco Bay Area) - Public demo key available

- API overview: [https://api.bart.gov/](https://api.bart.gov/)
- Use public key when user key is not provided: `MW9S-E7SL-26DU-VV8V`
- Station departures (etd):
  - `GET https://api.bart.gov/api/etd.aspx?cmd=etd&orig={station_abbr}&key=MW9S-E7SL-26DU-VV8V&json=y`
- Service advisories:
  - `GET https://api.bart.gov/api/bsa.aspx?cmd=bsa&key=MW9S-E7SL-26DU-VV8V&json=y`

#### 3) MTA (New York City) - API key commonly required

- Main portal: [https://api.mta.info/](https://api.mta.info/)
- If user did not provide a key, explain key requirement and offer setup guidance.
- If key is provided, use MTA GTFS-Realtime feeds for arrivals/service alerts.

### US Query Handling Rules / 미국 질의 처리 규칙

1. Detect city/system first: NYC(MTA), Boston(MBTA), SF Bay(BART), etc.
2. Use system-specific API and return:
   - next arrivals (minutes),
   - direction/destination,
   - service alerts (if available),
   - source URL.
3. If city is ambiguous, ask one short clarification question.
4. If API fails or endpoint is stale, use Tavily/web search to find current official API docs and retry.
5. Do not fabricate station codes, delays, or ETA.

### Quick API Examples / 빠른 API 예시

**MBTA (Boston) - South Station stops search**
```bash
curl "https://api-v3.mbta.com/stops?filter[name]=South%20Station"
```

**MBTA (Boston) - route list (subway only)**
```bash
curl "https://api-v3.mbta.com/routes?filter[type]=0,1"
```

**BART (SF) - Montgomery station departures**
```bash
curl "https://api.bart.gov/api/etd.aspx?cmd=etd&orig=MONT&key=MW9S-E7SL-26DU-VV8V&json=y"
```

### Fallback Message Template / 실패 시 안내 템플릿

**Korean:**  
`현재 해당 도시의 지하철 실시간 API를 확인할 수 없습니다. 공식 API 문서(또는 인증키) 확인 후 다시 조회할게요.`

**English:**  
`I can't reliably access that city's realtime subway API right now. I'll verify the official API docs (or required API key) and retry.`